...
Theorem Injection_Help' : forall Σ0 Σ ss ii opp s i op τ0 τ oplist σ η Ex Ex' θ θ',
  (Σ0 ||| mkSoup_Ing ss ii (mkop_cls opp τ0 :: oplist)) =
  (Σ |||  mkSoup_Ing s i (mkop_cls op τ :: σ)) ->
  ((Σ0 ||| << ss,ii+1,oplist>>) = (Σ ||| << s, i + 1, σ >>)) ->
  [[Ex, θ, Σ ||| << s, i, (op) __ (τ) :: σ >>  -- η --> Ex', θ', Σ ||| << s, i + 1, σ >> ]]
                                                                                               -> τ=τ0 .
Proof.
  intros Σ' Σ s' i' op' s i op τ' τ σ' σ η Ex Ex' θ θ'.
  intros H1 H2 H3.
  remember  (<< s', i', (op') __ (τ') :: σ' >> ) as Ing.
  remember  ( << s, i, (op) __ (τ) :: σ >>) as Ing'.
  remember ( << s', i' + 1, σ' >>) as opIng.
  remember (<< s, i + 1, σ >>) as opIng'.
...
Theorem Injection_Help_CC : forall Σ0 Σ ss ii opp s i op  τ oplist σ η Ex Ex' θ θ',
  (Σ0 ||| mkSoup_Ing ss ii (mkop_cls opp cc :: oplist)) =
  (Σ |||  mkSoup_Ing s i (mkop_cls op τ :: σ)) ->
  ((Σ0 ||| << ss,ii+1,oplist>>) = (Σ ||| << s, i + 1, σ >>)) ->
  [[Ex, θ, Σ ||| << s, i, (op) __ (τ) :: σ >>  -- η --> Ex', θ', Σ ||| << s, i + 1, σ >> ]]
                                                                                               -> τ=cc .
Proof.
  intros Σ' Σ s' i' op' s i op τ' τ σ' σ η Ex Ex' θ .
  intros H1 H2 H3.
...
Theorem Injection_Help_help : forall Σ Σ' ing ing', (Σ|||ing)=(Σ'|||ing')->(WF_union Σ' ing')
                                               -> (WF_union Σ ing) -> (Σ=Σ') /\ (ing=ing').
 Proof.
  intros Σ Σ' ing ing' H1 HWF' HWF.
  split.
  -Case" Σ = Σ'".
  rewrite Sess_Equal. intros I.
  rewrite Sess_Equal in H1. specialize  (H1 I).
  split.
   +SCase"->".
    destruct H1 as [Hr Hl].
    intro H.
    assert ( In Soup_Ing (Σ ||| ing) I). intuition. apply Hr in H0.
    unfold WF_union in HWF'. 
    inversion H0. apply H1.  inversion H1. rewrite H3 
  ...
  unfold WF_union in HWF'. unfold In in HWF'.
  -Case"ing = ing'".
  Qed.
