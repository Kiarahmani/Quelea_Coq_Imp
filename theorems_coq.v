...
Proof. 
    ...
    intros. generalize ( H3  x). intros.  assert (~(Ex) -hb x x). intuition. apply PaperH8 in H9. intuition. exact H7.
    clear H4 H5 H6 H8 H. inversion H0. assert ((E A vis so sameobj) = Ex). intuition. assert ((Ex)-A = A).
    rewrite <- H15. auto. assert ((Ex)-vis = vis). rewrite <- H15. auto. assert ((Ex)-so = so). rewrite <- H15. 
    auto. assert ((E A' vis' so' sameobj') = Ex'). intuition. assert ((Ex')-A = A'). rewrite <- H19. auto. 
    assert ((Ex')-vis = vis'). rewrite <- H19. auto. assert ((Ex')-so = so'). rewrite <- H19. auto. rewrite  H22 in H2. 
    ...
    assert (η.(seq)=i0). intuition. rewrite H28 in H2. apply Why_Coq in H2. auto. rewrite  H18 in H7. rewrite H16 in H7. 
